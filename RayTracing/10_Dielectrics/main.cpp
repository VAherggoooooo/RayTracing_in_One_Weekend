...
int main()
{
    ...
    const int samples_per_pixel = 600;//每个像素采样的次数 抗锯齿参数 同样也可以改变光追质量
    const int max_depth = 100;//最多反弹深度(次数) 光追采样参数
    ...
    shared_ptr<lambertian> material_ground = make_shared<lambertian>(color(0.6,0.1,0.1));
    shared_ptr<lambertian> material_center = make_shared<lambertian>(white);
    shared_ptr<metal> material_left = make_shared<metal>(white, 0.3);
    shared_ptr<metal> material_right = make_shared<metal>(white, 1.0);*/

    world.add(make_shared<sphere>(point3(0.0, -100.5, -1.0), 100.0, material_ground));
    world.add(make_shared<sphere>(point3(0.0, 0.0, -1.0), 0.5, material_center));
    world.add(make_shared<sphere>(point3(-1.0, 0.0, -1.0), 0.5, material_left));
    world.add(make_shared<sphere>(point3(-1.0, 0.0, -1.0), -0.4, material_left));//hollow glass
